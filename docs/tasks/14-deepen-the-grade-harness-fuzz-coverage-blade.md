...
3. **Small contract fixes**: (a) ~~(a) `is_versor` (grade.rs:59-66) claims "never says true for a non-versor" but returns true for Basis(8)=e0 (null, non-invertible) — guard `*i != 8` or reword; soundness unaffected, the doc claim is false as written.~~ **Discharged by SPEC-0020 §2.1** (2026-09-05): `is_versor` is deleted; the flag lives as `Analysis::versor`, documented as "the sandwich rule preserves the operand's grade set" — which `e₀` satisfies soundly — not as "is a versor". (b) `GradeSet` has no subset method — grade.rs:77 and the soundness test both hand-roll it; add a helper (or upstream to garust). (c) `Tensor::zeros/target` panic via `expect("tensor capacity overflow")` on adversarial dims (tensor.rs:14-17,53, merged #45) — §6 demands a typed error or a spec-recorded justification.
