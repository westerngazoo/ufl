...
| 2026-06-12 (impl) | **`RankDecomposition` generalized to `for_target(target, rank)`** so the engine can verify against the *arbitrary planted target* (AC3), not only `target(n)`. `new(n, rank)` (matmul) unchanged; `discharge`/`residual` contract identical; the `DimMismatch` error's `n` is computed `= target.dim().isqrt()` (exact for matmul, a harmless label otherwise — unreachable in the engine). | Gap found at scaffold time: AC3's planted target is **not** a matmul tensor, but the R-0007 predicate hardwired `target(n)`. The three-lens focused on GA mechanics and missed it. R-0007's 19 tests + doctest stay green. |
