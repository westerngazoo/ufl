...
| [R-0013](requirements/0013-matmul-moonshot.md) | **The matmul-decomposition moonshot** (the relocated Strassen prize) — discover a new exact decomposition (rank below known) by a **stronger, non-blind proposer** over the R-0006/7/8 seam + exact verifier. **Gate 0** (go/no-go): rediscover an exact rank-7 `T_2` decomposition where blind GA got **0/10**. The self-rewriting/Closure-Principle ideas are the proposer-strength lever. | Draft (owner-set moonshot; first live-verifier probe inconclusive → needs a fast-search harness + a real method: ALS-rounding / flip-graph / agentic) |
